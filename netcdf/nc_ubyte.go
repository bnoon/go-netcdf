...
func (v Var) ReadArrayUint8s(offsets []int, lens []int, data []uint8) error {
	dims, err := v.Dims()
	if err != nil {
		return err
	}
	...
	if u != UBYTE {
		return fmt.Errorf("wrong data type %s; expected %s", u, UBYTE)
	}
	if len(data) != total_len {
		return fmt.Errorf("Invalid length of recieving data %d (need %d)", len(data), total_len)
	}

	return newError(C.nc_get_vara_uchar(
...
func testReadUint8s(v Var, n uint64) error {
	...
	data = data[:12]
	if err := v.ReadArrayUint8s([]int{1, 1}, []int{-1, -1}, data); err != nil {
		return err
	}
	if data[0] != uint8(14) {
